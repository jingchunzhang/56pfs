...
static int add_tmp_to_task(t_pfs_tasklist *item)
{
	return 0;
	LOG(glogfd, LOG_NORMAL, "prepare add tmp task %s\n", item->task.base.filename);
	...
	t_pfs_tasklist *task0;
	if (pfs_get_task(&task0, TASK_Q_HOME))
	{
		LOG(glogfd, LOG_ERROR, "pfs_get_task err %m\n");
		return -1;
	}
	memcpy(&(task0->task), &(item->task), sizeof(t_pfs_taskinfo));
	if (item->task.sub.oper_type == OPER_SYNC_2_DOMAIN)
	{
		if (pfs_set_task(task0, TASK_Q_WAIT_SYNC))
		{
			LOG(glogfd, LOG_ERROR, "pfs_set_task err %m\n");
			return -1;
		}
	}
	else if (sub->need_sync == TASK_SYNC_ISDIR)
	{
		if (pfs_set_task(task0, TASK_Q_SYNC_DIR))
		{
			LOG(glogfd, LOG_ERROR, "pfs_set_task err %m\n");
			return -1;
		}
	...
	{
		if (pfs_set_task(task0, TASK_Q_WAIT))
		{
			LOG(glogfd, LOG_ERROR, "pfs_set_task err %m\n");
			return -1;
		}
...
int init_load_tmp_status()
{
	...
	snprintf(tmpfile, sizeof(tmpfile), "%s/pfs_tmp_status", datadir);
	struct stat filestat;
	if (stat(tmpfile, &filestat))
	{
		exist_tmp = 0;
		LOG(glogfd, LOG_ERROR, "%s not exit init it!\n", tmpfile);
		if (sub_init_tmp_status(tmpfile, sizeof(t_pfs_tasklist) * DEFAULT_ITEMS))
			return -1;
	}
	...
	for (i = 0; i < init_size; i++)
	{
		...
		{
			if (n < 0)
				LOG(glogfd, LOG_ERROR, "add_tmp_to_task [%s] err %m!\n", item.task.base.filename);
			set_tmp_blank(pos, NULL);
		}
	}
	if (exist_tmp)
...
int set_task_to_tmp(t_pfs_tasklist *tasklist)
{
	return 0;
	LOG(glogfd, LOG_NORMAL, "set_task_to_tmp %s\n", tasklist->task.base.filename);
	...
	list_for_each_entry_safe_l(tmp, l, &idletmp, list)
	{
		...
		ret = write(tmp_status_fd, tasklist, sizeof(t_pfs_tasklist));
		if (ret < 0)
			LOG(glogfd, LOG_ERROR, "write err %m\n");

		LOG(glogfd, LOG_DEBUG, "%s:%ld\n", tasklist->task.base.filename, tmp->pos);
	...
	if (sub_init_tmp_status(tmpfile, filestat.st_size + sizeof(t_pfs_tasklist)))
	{
