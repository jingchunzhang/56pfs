...
static void do_send(int fd)
{
	...
	if (ret == SEND_ADD_EPOLLIN && solib.svc_send)
		...
		{
			LOG(glogfd, LOG_DEBUG, "%s:%s:%d send close\n", ID, FUNC, LN);
			do_close(fd);
		}
}
...
int pfs_signalling_thread(void *arg)
{
	...
	if (argp->port > 0)
	{
		...
		n = 1;
		setsockopt(lfd, SOL_TCP, TCP_NODELAY, &n, sizeof(n));
		n = 0;
		setsockopt(lfd, SOL_TCP, TCP_CORK, &n, sizeof(n));
