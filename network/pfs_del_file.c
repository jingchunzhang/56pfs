...
{
	char * datadir = myconfig_get_value("pfs_deldir");
	if (datadir == NULL)
		datadir = "/diska/pfs/path/delfile/";
...
int add_2_del_file (t_task_base *base)
{
	...
	t_pfs_del_file del_file;
	memset(&del_file, 0, sizeof(del_file));
	snprintf(del_file.dtime, sizeof(del_file.dtime), "%ld", base->mtime);
	del_file.idle = ' ';
