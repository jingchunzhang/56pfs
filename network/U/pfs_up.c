...
#include "pfs_up_sub.c"

int svc_init() 
{
	char *logname = myconfig_get_value("log_up_logname");
	if (!logname)
		logname = "../log/up_log.log";

	char *cloglevel = myconfig_get_value("log_up_loglevel");
	int loglevel = LOG_NORMAL;
	if (cloglevel)
		loglevel = getloglevel(cloglevel);
	int logsize = myconfig_get_intval("log_up_logsize", 100);
	int logintval = myconfig_get_intval("log_up_logtime", 3600);
	int lognum = myconfig_get_intval("log_up_lognum", 10);
	pfs_up_log = registerlog(logname, loglevel, logsize, logintval, lognum);
	if (pfs_up_log < 0)
		return -1;
	LOG(pfs_up_log, LOG_NORMAL, "svc_init init log ok!\n");
...
int svc_initconn(int fd) 
{
	...
	LOG(pfs_up_log, LOG_TRACE, "%s:%s:%d\n", ID, FUNC, LN);
	struct conn *curcon = &acon[fd];
	if (curcon->user == NULL)
		curcon->user = malloc(sizeof(pfs_cs_peer));
	if (curcon->user == NULL)
	{
		LOG(pfs_up_log, LOG_ERROR, "malloc err %m\n");
		char val[256] = {0x0};
	...
	MD5Init(&(peer->ctx));
	LOG(pfs_up_log, LOG_TRACE, "a new fd[%d] init ok!\n", fd);
	return 0;
}
...
{
	LOG(pfs_up_log, LOG_DEBUG, "%s:%s:%d\n", ID, FUNC, LN);
	t_pfs_sig_head h;
	t_pfs_sig_body b;
	char *data;
	size_t datalen;
	...
	{
		LOG(pfs_up_log, LOG_TRACE, "%s:%d fd[%d] no data!\n", FUNC, LN, fd);
		return -1;  /*no suffic data, need to get data more */
	}
	int ret = parse_sig_msg(&h, &b, data, datalen);
	if (ret > 0)
	{
		LOG(pfs_up_log, LOG_TRACE, "fd[%d] no suffic data!\n", fd);
		return -1;  /*no suffic data, need to get data more */
	}
	if (ret == E_PACKET_ERR_CLOSE)
	{
		LOG(pfs_up_log, LOG_ERROR, "fd[%d] ERROR PACKET bodylen is [%d], must be close now!\n", fd, h.bodylen);
		return RECV_CLOSE;
	}
...
recvfileing:
	peer->hbtime = time(NULL);
	list_move_tail(&(peer->alist), &activelist);
	LOG(pfs_up_log, LOG_TRACE, "fd[%d] sock stat %s!\n", fd, sock_stat_cmd[peer->sock_stat]);
	if (peer->sock_stat == RECVFILEING)
	{
		...
		if(task0 == NULL)
		{
			LOG(pfs_up_log, LOG_ERROR, "recv task is null!\n");
		...
		LOG(pfs_up_log, LOG_TRACE, "fd[%d] get file %s:%d!\n", fd, task->base.filename, datalen);
		...
		if (task->sub.lastlen >= task->sub.processlen)
		{
			...
			{
				LOG(pfs_up_log, LOG_ERROR, "fd[%d] get file %s error!\n", fd, task->base.filename);
...
void svc_timeout()
{
	...
	list_for_each_entry_safe_l(peer, l, &activelist, alist)
	{
		...
		if (now - peer->hbtime < to)
			break;
		LOG(pfs_up_log, LOG_DEBUG, "timeout close %d [%lu:%lu] %d\n", peer->fd, now, peer->hbtime, peer->role);
		do_close(peer->fd);
	}
}
...
{
	LOG(pfs_up_log, LOG_TRACE, "close %d\n", fd);
	struct conn *curcon = &acon[fd];
	if (curcon->user == NULL)
		return;
	pfs_cs_peer *peer = (pfs_cs_peer *) curcon->user;
	...
	if (peer->sendtask)
	{
		...
		LOG(pfs_up_log, LOG_ERROR, "abort send %s!\n", tasklist->task.base.filename);
