...
{
	LOG(pfs_up_log, LOG_DEBUG, "send %d cmdid %x\n", peer->fd, h->cmdid);
	char obuf[2048] = {0x0};
	int n = 0;
...
static int do_req(int fd, t_pfs_sig_head *h, t_pfs_sig_body *b)
{
	...
	switch(h->cmdid)
	{
		case CMD_GET_FILE_RSP:
			...
			{
				LOG(pfs_up_log, LOG_ERROR, "fd[%d] recv %s a bad bodylen[%d:%d]!\n", fd, str_cmd[h->cmdid], h->bodylen, sizeof(t_task_base));
				return RECV_CLOSE;
			}
			LOG(pfs_up_log, LOG_DEBUG, "fd[%d] recv %s %s\n", fd, str_cmd[h->cmdid], base->filename);
		...
		default:
			LOG(pfs_up_log, LOG_ERROR, "fd[%d] bad cmd close it [%x]\n", fd, h->cmdid);
			return RECV_CLOSE;
	}
